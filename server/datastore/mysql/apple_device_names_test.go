...
func testHostDeviceNamesSummaryAndFilter(t *testing.T, ds *Datastore) {
	...
	// comboHost has a verified config profile AND a failed rename: the shared
	// status CASE must let the failed rename win (failed > verified precedence).
	...
	// Give comboHost a separate verified (install) config profile so its status is
	// determined by combining the profile and device-name buckets in the CASE.
	_, err = ds.writer(ctx).ExecContext(ctx, `
