...
#
# Spotify installs per user and has no machine-wide mode, so installing it from this
...
try {
    ...
    if (-not $owner) {
        Throw "Spotify installs per user and no user is signed in to this host. Sign in and try again."
    }
    $userAccount = "$($owner.Domain)\$($owner.User)"
    Write-Host "Installing Spotify for $userAccount."
